# ADR-002: Data Persistence via Fusion Attributes

## Status
Accepted

## Context
The add-in needs to persist the system diagram data (blocks, connections, layout) so that it travels with the Fusion design file (`.f3d`).
Options considered:
1.  **External File:** Save a separate `.json` file alongside the `.f3d` file.
2.  **Custom Data:** Use Fusion's `attributes` API to embed data directly into the component.
3.  **Cloud Storage:** Save data to an external cloud database linked by Document ID.

## Decision
We chose **Option 2: Custom Data (Fusion Attributes)**.
...
- **Version Control:** Fusion's native versioning handles the data. Reverting to a previous version of the design also reverts the diagram state.
...
- **Opaque Data:** The data is hidden inside the binary `.f3d` file and cannot be easily inspected without opening Fusion (unlike a plain text file).
