...
class DiagnosticsRunner:
    ...
    # NOTE: Diagnostic tests must never modify the user's document.
    # Earlier versions created a temporary component and sketch here to
    # verify write access, which dirtied the document and polluted its
    # undo timeline. Document write access is now verified non-invasively
    # by test_fusion_attribute_read_write below — attributes are the only
    # thing this add-in ever writes to a design.
    # cleanup_any_remaining_temp_objects() is retained to remove leftovers
    # created by those older versions.
